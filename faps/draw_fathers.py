...
def draw_fathers(partition, paternity_probs, null_probs=None, ndraws=1000, max_tries=100):
    """
    ...
    while len(output) < 1:
        ...
        if counter == max_tries:
            print "Could not find any Monte Carlo draws compatible with the partition after {} tries.".format(max_tries)
            break
        

    if null_probs is not None:
        if null_probs == 'uniform':
            null_probs = np.array([1.0/(nfathers-1)] * (nfathers-1))
        elif len(null_probs) != (nfathers-1):
            print "Warning: Length of null_probs does not match the number of fathers (number of columns in paternity_probs -1)."

        null_samples = np.array([np.random.choice(nfathers-1, ndraws, replace=True, p = null_probs) for i in range(nfamilies)])
        null_samples = null_samples.T
        # identify samples with two or more famililies with shared paternity
        counts = [np.unique(null_samples[i], return_counts=True)[1] for i in range(len(null_samples))]
        valid  = [all((counts[i] == 1) & (counts[i] != nfathers))   for i in range(len(counts))]
        null_samples = null_samples[valid]
        null_samples = [val for sublist in null_samples for val in sublist]
        return [output, null_samples]
    else:
        return output
